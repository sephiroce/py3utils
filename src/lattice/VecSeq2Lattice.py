...
latfile = open("%s.lat.txt"%name,"w")
latfile.write("%s\n"%name)
latfile.write("0 1 0 0.0,0.0,\n")
for t in range(len(nt)):
  P = pow(nb, t)
  for p in range(P):
    for ol in nt[t].argsort()[Nbest:][::-1]:
      state = state + 1
      latfile.write("%d %d %d 0.0,%.6f,\n"%(state / nb, state, ol,nt[t][ol]))
      if t == len(nt) - 1:
        latfile.write("%d %d 0 0.0,0.0,\n"%(state, final_state))
latfile.write("%d 0.0,0.0,\n"%final_state)
